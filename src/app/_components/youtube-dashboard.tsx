...
export function YouTubeDashboard({ signedIn }: { signedIn: boolean }) {
	...
	const threads =
		(commentsQuery.data as { items?: CommentThread[] })?.items ?? [];
	const myChannelId = (myChannelQuery.data as string | null) ?? null;
	const isOwner = !!(
		video?.snippet?.channelId &&
		myChannelId &&
		video?.snippet?.channelId === myChannelId
	);
			...
			{video && (
				<Card className="mb-6 p-4">
					<div className="flex flex-col gap-4">
						{(() => {
							const vid =
								(video as unknown as { id?: string })?.id ||
				...
				<div className="grid gap-6 lg:grid-cols-2">
						...
						{commentsQuery.error && (
							<Alert className="mb-4">
								...
								<AlertDescription className="flex items-center gap-2">
									{commentsQuery.error.message.includes(
						...
						{!commentsQuery.error && (
							<>
								...
								<div className="mt-4 space-y-4">
									...
									{(() => {
										const nextToken = (
											commentsQuery.data as { nextPageToken?: string }
										)?.nextPageToken;
										return (
												...
												<Button
													...
													onClick={() => {
														if (prevCommentTokens.length) {
															...
															setCommentsPageToken(
																prev?.length ? prev : undefined,
															);
														}
													}}
													disabled={
														!prevCommentTokens.length || commentsQuery.isLoading
													}
												>
													Previous
												</Button>
					...
					<Card className="p-4">
						...
						<div className="mt-3 flex gap-2">
							...
							<Button
								variant="secondary"
								onClick={() => deleteNoteMutation.mutate({ videoId })}
